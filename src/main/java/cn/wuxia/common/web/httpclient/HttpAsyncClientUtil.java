...
public class HttpAsyncClientUtil {
     ...
     */
    private static void setSsl(HttpAsyncClientBuilder builder) throws HttpClientException {
        try {
     ...
     * @author songlin
     */
    private static PoolingNHttpClientConnectionManager getConnectionManager() throws HttpClientException {
...
//        }
     ...
     * @author songlin
     */
    public static List<HttpAsyncClientResponse> call(HttpAsyncClientRequest... param) throws HttpClientException {
        CloseableHttpAsyncClient httpclient = getHttpClient(param[0]);
        List<HttpAsyncClientResponse> resps = Lists.newArrayList();
        try {
            // Start the client
            httpclient.start();
            ...
            Map<String, Future<org.apache.http.HttpResponse>> futureMap = Maps.newConcurrentMap();
            for (final HttpAsyncClientRequest clientRequest : param) {
                ...
                try {
                    ...
                    httpRequest.addHeader("requestId", clientRequest.getRequestId());
                    /**
                    ...
                    logger.info("async executing request {}, {}", clientRequest.getRequestId(), httpRequest.getRequestLine());
                    futureMap.put(clientRequest.getRequestId(), future);
                } catch (Exception e) {
                    logger.error("", e);
                }
            }
            try {
                latch.await();
                for (Map.Entry<String, Future<org.apache.http.HttpResponse>> futureEntry : futureMap.entrySet()) {
                    try {
                        resps.add(getResponse(futureEntry));
            ...
            logger.info("完成异步请求，关闭连接, httpclient.isRunning:{}", httpclient.isRunning());
//            HttpAsyncClientUtils.closeQuietly(httpclient);
     ...
     * @author songlin
     */
    public static List<HttpAsyncClientResponse> postTexts(String url, String... text) throws HttpClientException {
        HttpClientRequest param = HttpAsyncClientRequest.post(url);
        ...
        try {
            ...
            for (String t : text) {
                try {
                    ...
                    httppost.addHeader("requestId", i + "");
                    // 创建参数队列  
...
//            HttpAsyncClientUtils.closeQuietly(httpclient);
     ...
     * @author songlin
     */
    public static HttpAsyncClientResponse getResponse(Map.Entry<String, Future<org.apache.http.HttpResponse>> futureEntry) throws HttpClientException {
        HttpAsyncClientResponse resp = new HttpAsyncClientResponse();
        try {
            resp.setRequestId(futureEntry.getKey());
            org.apache.http.HttpResponse response = futureEntry.getValue().get(5, TimeUnit.SECONDS);
            ...
            if (entity != null && statusCode == HttpStatus.SC_OK) {
                ...
                throw new HttpClientException("[" + statusCode + "]" + response.getStatusLine().getReasonPhrase());
            }
