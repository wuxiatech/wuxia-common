...
public class Pages<T> implements Serializable {
     ...
     * set order mode.
     *
     * @param sortOrder 's value is desc or asc, when order is more than one
    ...
    public void setSortOrder(final String sortOrder) {
        ...
        for (String orderStr : orders) {
            if (!StringUtils.equalsIgnoreCase(Sort.Direction.DESC.toString(), orderStr)
                    && !StringUtils.equalsIgnoreCase(Sort.Direction.ASC.toString(), orderStr)) {
    ...
    public Sort getSort() {
        if (isOrderBySetted()) {
            ...
            for (int i = 0; i < orderByArray.length; i++) {
                if (StringUtils.equalsIgnoreCase(Sort.Direction.ASC.toString(), orderArray[i])) {
                    orders.add(new Sort.Order(Sort.Direction.ASC, orderByArray[i]));
                } else {
                    orders.add(new Sort.Order(Sort.Direction.DESC, orderByArray[i]));
                }
            }
    ...
    /**
     * @param conditions <ConditionsBean>
     */
