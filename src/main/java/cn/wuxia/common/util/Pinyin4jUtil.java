...
public class Pinyin4jUtil {
    ...
    public static String getPinyin(String chinese) {
        return StringUtil.join(makeStringByStringSet(chinese), ",");
    }
    ...
    public static String getPinyinToUpperCase(String chinese) {
        return StringUtil.join(makeStringByStringSet(chinese), ",").toUpperCase();
    }
    ...
    public static String getPinyinToLowerCase(String chinese) {
        return StringUtil.join(makeStringByStringSet(chinese), ",").toLowerCase();
    }
    ...
    public static Set<String> makeStringByStringSet(String chinese) {
        ...
        if (StringUtil.isNotBlank(chinese)) {
            ...
            for (int i = 0; i < srcChar.length; i++) {
                ...
                // 去除重复
                Set<String> yigezi = new HashSet<String>();
                for (int j = 0; j < temp[i].length; j++) {
                    yigezi.add(temp[i][j]);
                }
                temp[i] = yigezi.toArray(new String[] {});
            }
            String[] pingyinArray = Exchange(temp);
            // 去除重复
            Set<String> zhongWenPinYin = new HashSet<String>();
            for (int i = 0; i < pingyinArray.length; i++) {
                zhongWenPinYin.add(StringUtil.replace(pingyinArray[i], "u:", "v"));
            }
    ...
    public static String getPinyinConvertJianPin(String chinese) {
        ...
        Set<String> strChars = Sets.newHashSet();
        for (String str : strArray) {
            ...
            strChars.add(strChar);
        }
        return StringUtil.join(strChars, ",");
    }
    ...
    public static void main(String[] args) {
        ...
        String pinyin = getPinyinJianPin("大兴安岭地区松岭区");
        System.out.println(pinyin);
        if (pinyin.length() > 200) {
            pinyin = StringUtil.substring(pinyin, 0, 200);
            int endindex = pinyin.lastIndexOf(",");
            pinyin = StringUtil.substring(pinyin, 0, endindex);
        }
        System.out.println(pinyin);
