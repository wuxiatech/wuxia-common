...
public class HibernateUtils {
    ...
    /**
     * 根据对象ID集合, 整理合并集合. 页面发送变更后的子对象id列表时,
     * 在Hibernate中删除整个原来的子对象集合再根据页面id列表创建一个全新的集合这种看似最简单的做法是不行的.
     ...
     */
    public static <T> void mergeByCheckedIds(final Collection<T> srcObjects, final Collection<String> checkedIds, final Class<T> clazz)
            throws Exception {
        ...
        try {
            ...
            while (srcIterator.hasNext()) {
                ...
                String idName = getIdName(element.getClass());
                Long id = (Long) ReflectionUtil.invokeGetterMethod(element, idName);
            ...
            for (String id : checkedIds) {
                ...
                String idName = getIdName(element.getClass());
                ReflectionUtil.invokeSetterMethod(element, idName, id, String.class);
    ...
    public static String getIdName(Class<?> clazz) throws IntrospectionException {
