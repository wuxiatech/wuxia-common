package cn.wuxia.qiyukf.openapi.session;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import cn.wuxia.qiyukf.openapi.session.constant.EventType;
import cn.wuxia.qiyukf.openapi.session.model.QiyuMessage;
import cn.wuxia.qiyukf.openapi.session.model.QiyuSession;
import cn.wuxia.qiyukf.openapi.session.util.MD5;
import cn.wuxia.qiyukf.openapi.session.util.QiyuPushCheckSum;
...
public abstract class ResponseParser {
     ...
     * @return 固定返回为空字符串
     */
    public String onReceive(Long time, String checksum, String eventType, String content, String domain) {
        try {
            ...
            switch (eventType) {
                case EventType.MSG: {
                    ...
                    message.setMsgOrigin(domain);
                    onMessage(message);
                }
                    break;
                case EventType.SESSION_START: {
                    QiyuSession session = JSON.toJavaObject(json, QiyuSession.class);
    ...
    protected abstract void onSessionStart(QiyuSession session);

    protected abstract void onSessionEnd(QiyuSession session);
